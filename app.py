...
if _sub_pendientes:
    with _sub_pendientes:
        ...
        def _pendientes_frag():
            ...
            def _nota_key(f):
                cuit = f.get("cuit") or ""
                cliente = cuit if cuit else (f.get("apellido_razon_soc") or "")
                return (cliente, f.get("total"), f.get("fecha_comp"))
            _notas_keys = {_nota_key(f) for f in facturas_bal if "NOTA_CREDITO" in str(f.get("tipo_comp") or "").upper()}
            _fac_deud  = [f for f in facturas_bal    if str(f.get("anulada","N")).upper() != "S" and "NOTA" not in str(f.get("tipo_comp") or "").upper() and _nota_key(f) not in _notas_keys and _pend_saldo_f(f) > 0 and _pend_en_rango(f.get("fecha_comp"))]
...
if _sub_percibido:
  ...
  <div style='display:grid;grid-template-columns:{"1fr 1fr 1fr 1fr" if total_aj_neg_p else "1fr 1fr 1fr"};gap:16px'>
    ...
    {_metric_cell("Ajustes (−)", f"$ {_pesos(abs(total_aj_neg_p))}", "#c62828") if total_aj_neg_p else ""}
            ...
            if _pagos_rango:
                ...
                _comp_tot_lkp = {str(c.get("nro_comprobante") or ""): float(c.get("total") or 0) for c in comprobantes_bal if c.get("nro_comprobante")}
                _pag_por_comp = {}
                for _px in pagos_bal:
                    for _ix in (_px.get("imputaciones") or []):
                        _nro = str(_ix.get("nro_comprobante") or "")
                        if _nro:
  ...
  <div style='text-align:center;color:#666;font-size:0.9em'>Ingresos percibidos − Egresos percibidos</div>
            ...
            # ── DEVOLUCIONES PRÉSTAMOS (siempre visible) ─────────────────────────────
            st.divider()
    ...
    with _stab_otros_egresos:
        ...
        with _oe_tab2:
            ...
            def _oe_editar_eliminar():
                ...
                _rubros_en_lista = sorted({(o.get("rubros_egresos") or {}).get("nombre") or "—" for o in _lista})
                _fil_rubro = st.selectbox("Rubro", ["Todos"] + _rubros_en_lista, key="oe_fil_rubro")
                if _fil_rubro != "Todos":
                    _lista = [o for o in _lista if (o.get("rubros_egresos") or {}).get("nombre") == _fil_rubro]
                _subs_en_lista = sorted({(o.get("subrubros_egresos") or {}).get("nombre") or "—" for o in _lista})
                _fil_sub = st.selectbox("Subrubro", ["Todos"] + _subs_en_lista, key="oe_fil_sub")
                if _fil_sub != "Todos":
                    _lista = [o for o in _lista if (o.get("subrubros_egresos") or {}).get("nombre") == _fil_sub]
                _items_en_lista = sorted({(o.get("items_egresos") or {}).get("nombre") or "—" for o in _lista})
                _fil_item = st.selectbox("Item", ["Todos"] + _items_en_lista, key="oe_fil_item")
                if _fil_item != "Todos":
                    _lista = [o for o in _lista if (o.get("items_egresos") or {}).get("nombre") == _fil_item]
                ...
                with st.form("form_cobros_wix_cajas", border=True):
                    ...
                    for _o in _wix_sorted_cob:
                        ...
                        with st.container(border=True):
                            ...
                            _cd, _cf, _cc = st.columns([2, 3, 3])
                            _cobrado = _cd.checkbox(
                                "Cobrado",
                                value=(_fecha_pago_val is not None),
                                key=f"cobrado_{_oid_c}",
                            )
                            _fp_raw = _cf.date_input(
                                "F. pago",
    ...
    def _diario_frag():
        ...
        if str(f0) not in (fechas_actuales or []):
            ...
            if not rows:
                ...
                if not _sin_rubro_a_avisar.empty:
                    _prods_sin_rubro = sorted(_sin_rubro_a_avisar["Producto"].unique())
                    st.warning(
